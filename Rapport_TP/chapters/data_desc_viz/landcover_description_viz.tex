\section{Land Cover data description and visualization}
...
\begin{enumerate}
    \item \textbf{Distribution Map:} The Land Cover Map (Figure \ref{fig:landcover\_map}) was plotted, coloring polygons based on their $\texttt{lcccode}$ to visualize the spatial arrangement of classes.
    \item \textbf{Proportional Distribution:} A pie chart (Figure \ref{fig:lcccode\_pie}) displayed the percentage distribution of each unique $\texttt{lcccode}$.
    \item \textbf{Area Analysis:} The geometric area of each polygon was calculated ($\texttt{landcover\_gdf['area'] = ...geometry.area}$). A bar plot (Figure \ref{fig:lcccode\_area}) was generated showing the **Total Area by Land Cover Code**.
\end{enumerate}

\begin{figure} [!h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/LandCover/Land Cover Types - Algeria and Tunisia Merged.png}
    ...
    \label{fig:landcover\_map}
\end{figure}

\begin{figure} [!h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/LandCover/total area by lcc code bar chart.png}
    \caption{Total aggregated area (in square degrees) covered by each Land Cover Code in the combined dataset.}
    \label{fig:lcccode\_area}
\end{figure}
...
Given the 22 unique $\texttt{lcccode}$ values in the dataset, a **high-level, simplified mapping** was created to aggregate these codes into fundamental categories (e.g., "Forests," "Bare lands," "Croplands," "Vegetation," etc.). This step is crucial for preparing the Land Cover feature for the predictive model by reducing the number of target classes, thus addressing the complexity of mixed classification codes. The final mapping dictionary was saved for use in the feature engineering phase.
...
\subsection{Data Preparation and Raster Clipping Workflow}

The preprocessing workflow began with the configuration of environment variables to manage dataset paths and ensure portability across processing stages. The geographical area of interest (AOI) was defined by loading merged country boundaries from a GeoPackage file (\texttt{aoi\_dza\_tun.gpkg}) into a \texttt{GeoDataFrame} using \texttt{geopandas}. In parallel, the source elevation raster was loaded from the \texttt{be15\_grd} directory using \texttt{rasterio}, allowing access to its spatial metadata, including the native Coordinate Reference System (CRS).

To enable accurate spatial operations, the vector boundaries were reprojected to match the raster’s CRS. This alignment step is essential, as raster masking requires both datasets to share the same spatial reference. Once the coordinate systems were unified, the elevation raster was clipped using the AOI geometries through the \texttt{rasterio.mask.mask()} function, producing a masked elevation array and an updated spatial transformation.

Finally, the original raster metadata was updated to reflect the new dimensions and georeferencing parameters of the clipped dataset. The output format was explicitly set to \texttt{GeoTIFF} to ensure write compatibility and standardization. The resulting digital elevation model was saved as \texttt{elevation\_dz\_tn.tif}, constituting the final processed dataset ready for subsequent topographical or hydrological analyses.

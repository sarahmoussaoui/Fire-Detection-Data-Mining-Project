...
\subsubsection{From-Scratch KNN Model Evaluation}

\paragraph{Confusion Matrices (Training, Validation, and Test Sets)}
The training confusion matrix shows perfect classification, with no false positives or false negatives, indicating complete memorization of the training data. In contrast, the validation and test confusion matrices reveal a more realistic performance. Both sets exhibit a non-negligible number of false positives, while false negatives remain relatively limited. This behavior reflects a model that prioritizes sensitivity to fire events at the expense of precision, a common outcome in imbalanced classification settings. The similarity between validation and test error patterns suggests stable generalization behavior.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{images/KNN/confusion_matrices_initial_from_scratch.png}
    \caption{Confusion matrices for the from-scratch KNN model.}
    \label{fig:confusion_matrices_knn_from_scratch}
\end{figure}

\paragraph{ROC Curve Analysis}
The ROC curves further confirm these observations. The training ROC curve achieves an AUC of 1.00, reinforcing the presence of overfitting. However, the validation and test ROC curves achieve AUC values of approximately 0.885 and 0.864, respectively, indicating good discriminative ability on unseen data. Both curves remain well above the random classifier baseline, demonstrating that the from-scratch KNN model effectively ranks fire-prone and non-fire observations despite reduced threshold-dependent performance.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{images/KNN/roc_curves_initial_from_scratch.png}
    \caption{ROC curves for the from-scratch KNN model.}
    \label{fig:roc_curves_knn_from_scratch}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{images/KNN/metrics_comparison_KNN_predefined.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{images/KNN/confusion_matrices_KNN_predefined.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{images/KNN/threshold_optimization_bayesian_KNN_predefined.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{images/KNN/predefined_vs_optimized_comparison_KNN.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{images/KNN/bayesian_optimization_history_KNN.png}
